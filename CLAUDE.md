...
- **章の区切り**: 上記 + 作業中の参考書別設計書を全文再読込
- **品質ゲートの手順を正確に列挙できなくなったら**: 作業を止めて全指示書を再読込

### チェックポイント書き出し
- **3セクション完了ごと / 章の区切り**: `checkpoint.yaml` を更新
- **各セクション完了時**: `manifest.yaml` を更新
- **判断に迷った時**: `decisions.yaml` に記録
- 詳細: [構造化共通ワークフロー §10](docs/design/構造化共通ワークフロー.md)

### プリフライトチェック（各セクション着手前）
- 前セクションが全ゲート通過済みか確認
- ノード抽出ルール・品質ゲート・情報保存ルールを覚えているか自己チェック
- 覚えていなければ該当ドキュメントを再読込してから着手
