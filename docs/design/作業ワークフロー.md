...
> **⚠️ 本書は旧版です。新しい参考書の構造化には以下のドキュメントを使用してください:**
> - [構造化共通ワークフロー](構造化共通ワークフロー.md)（全参考書共通の手順・品質基準）
> - [参考書別設計テンプレート](参考書別設計テンプレート.md)（参考書ごとの設計書フォーマット）
> - [チーム運用ルール](チーム運用ルール.md)（並列作業・レビュー・情報共有）
>
> 本書は「はじめの英文読解ドリル」の構造化で使用した初版ワークフローであり、
> 歴史的参照として残しています。
